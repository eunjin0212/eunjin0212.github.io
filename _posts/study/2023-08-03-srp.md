...
컴포넌트란 무엇일까? 필자가 정의한 컴포넌트는 재사용이 가능하게 UI 단위로 만든 벽돌 같은 것이다.
...
좋은 구조를 만드려면 컴포넌트가 잘 나누어져야 한다. 컴포넌트를 나누는 방법에는 여려가지가 있겠지만, 필자는 무언가 반복되는 것이 두 개 이상일 경우 컴포넌트로 분리하는 편이다.

예를 들어 지금의 계산기 페이지와 다른 페이지에 연산자 버튼들이 사용되어야 한다면? 두 페이지에 각각 연산자 버튼들을 추가 하면 중복 코드가 작성된다. 그리고 나머지를 계산하는 연산을 추가해야 한다면, 두 페이지에 가서 나머지 계산식을 넣어야 한다. 기능이 담긴 컴포넌트를 새로 만든다면 중복을 줄이고 유지보수성을 높일 수 있다.
...
[예제 코드 플레이그라운드](https://url.kr/cn2ger)
